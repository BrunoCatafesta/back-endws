...
public class InstallmentCalculatorTest {
    ...
    public void mustReturnInstallmentValueTenForOrderedWithProductAndOneInstallment(){
        ...
        Product product1 = new ProductBuilder()
                .withBarcode(363462346457L)
                .withDescription("Test product")
                .withName("Test")
                .withUnitPrice(installmentValueExpected)
                .builde();
        Mockito.when(productRepository.find(Mockito.any(Product.class))).thenReturn(Optional.ofNullable(product1));
        
        OrderedItem orderedItem = new OrderedItemBuilder()
				.withProduct(product1)
				.withUnits(BigDecimal.ONE)
				.builde();
        ...
        Ordered ordered = new OrderedBuilder()
        		.withProducts(Collections.singletonList(orderedItem))
        		.withPayment(payment)
        		.builde();
